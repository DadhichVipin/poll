...
def polls_list(request):
    ...
    return render(request, 'polls/polls_lists.html', context)
...
def polls_add(request):
    if request.user.has_perm('polls.add_poll'):
        count_polls = Poll.objects.filter(owner=request.user).count()

        if request.method == 'POST':
            form = PollAddForm(request.POST)
            if count_polls == 5:
                messages.warning(
                    request, "Sorry You already 5 Poll added..... ", extra_tags='alert alert-warning alert-dismissible fade show')
